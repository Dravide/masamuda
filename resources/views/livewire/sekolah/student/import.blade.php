    ...
    <div class="container">
            ...
            <div class="col-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('sekolah.siswa.index') }}" class="text-primary">Data Siswa</a></li>
        ...
        <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between mb-4">
            <div class="clearfix">
                ...
                <span class="text-muted">Upload file Excel untuk import data massal</span>
            </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control @error('file') is-invalid @enderror" wire:model="file" accept=".xlsx,.xls,.csv">
                ...
                <div class="card">
                                ...
                                <tbody>
                                    @foreach($previewData as $data)
                                        <tr class="{{ $data['status'] == 'Invalid' ? 'table-danger' : '' }}">
                                            ...
                                            <td>{{ $data['email'] ?? '-' }}</td>
                                            <td>{{ $data['whatsapp'] ?? '-' }}</td>
                                            <td>{{ $data['birth_date'] ? \Carbon\Carbon::parse($data['birth_date'])->locale('id')->translatedFormat('d F Y') : '-' }}</td>
                                            <td>
                    ...
                    <div class="card-footer border-top text-end">
                        ...
                        @if(count($validData) > 0)
                            <button wire:click="import" class="btn btn-primary waves-effect waves-light" onclick="confirm('Yakin ingin mengimport {{ count($validData) }} data valid?') || event.stopImmediatePropagation()">
